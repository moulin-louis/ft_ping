...
int send_echo(PING* ping) {
  ...
  if (data_buffer)
    ping_set_data(ping, data_buffer, off, data_length > off ? data_length - off : data_length, USE_IPV6);
...
int ping_finish(void) {
  ...
  if (ping->ping_num_xmit) {
    if (ping->ping_num_recv > ping->ping_num_xmit)
      ...
      printf("%d%% packet loss", (int)(((ping->ping_num_xmit - ping->ping_num_recv) * 100) / ping->ping_num_xmit));
  }
  printf("\n");
  return 0;
}
